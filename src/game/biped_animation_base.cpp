...
String BipedAnimationBase::get_upper_body_animation_state_string(const BipedCharacterUpperBodyState p_state) const {
	switch (p_state) {
		case RIFLE_IDLE: {
			return "WeaponIdle";
		} break;
		...
		case UPPER_BODY_STATE_SEQUENCE: {
			return "Sequence";
		} break;
		case UPPER_BODY_STATE_MAX: {
			DEV_ASSERT(false);
		} break;
	}
}
...
BipedCharacterUpperBodyState BipedAnimationBase::get_current_upper_body_animation_state() const {
	...
	const static StringName sequence_state = StringName("Sequence");
	...
	if (curr_state == rifle_aim_state) {
		return BipedCharacterUpperBodyState::RIFLE_AIM;
	} else if (curr_state == rifle_idle_state) {
		return BipedCharacterUpperBodyState::RIFLE_IDLE;
	} else if (curr_state == sequence_state) {
		return BipedCharacterUpperBodyState::UPPER_BODY_STATE_SEQUENCE;
	}

	ERR_FAIL_V_MSG(RIFLE_IDLE, vformat("Unknown upper body state %s!?", curr_state));
}

void BipedAnimationBase::set_upper_body_animation_state(const BipedCharacterUpperBodyState p_state) const {
...
void BipedAnimationBase::set_weapon_animation_set(const WeaponAnimationSetType &p_weapon_animation_type) {
	...
	model->get_animation_tree()->set(
			...
			model->get_animation_settings()->get_weapon_animation(p_weapon_animation_type, CharacterWeaponAnimations::IDLE));
	model->get_animation_tree()->set(
			"parameters/Locomotion/WeaponAim/animation",
			model->get_animation_settings()->get_weapon_animation(p_weapon_animation_type, CharacterWeaponAnimations::AIM));
	/*model->get_animation_tree()->set(
		"parameters/Locomotion/WeaponReload/animation",
		model->get_animation_settings()->get_weapon_animation(p_weapon_animation_type, BipedCharacterUpperBodyState::UPPER_BODY_STATE_SEQUENCE)
	);*/
...
Ref<AnimationSequenceFuture> BipedAnimationBase::trigger_sequence(const CharacterAnimationSequence p_sequence) {
	Ref<BipedAnimationSequenceFuture> out;
	StringName animation;
	switch (p_sequence) {
		case CharacterAnimationBase::CharacterAnimationSequence::SEQUENCE_RELOAD: {
			out.instantiate();
			out->sequence_slot = BipedCharacterAnimationSequenceSlot::SEQUENCE_SLOT_UPPER_BODY;
			animation = model->get_animation_settings()->get_weapon_animation(weapon_animation_set, CharacterWeaponAnimations::RELOAD);
		} break;
		default: {
			ERR_FAIL_V_MSG(nullptr, vformat("Unsupported sequence type %d!", p_sequence));
		} break;
	}

	DEV_ASSERT(out.is_valid());

	switch (out->sequence_slot) {
		case SEQUENCE_SLOT_UPPER_BODY: {
			out->node_path = "parameters/Locomotion/UpperBodySequence";
		} break;
	}
	model->get_animation_tree()->set(
			out->node_path + "/animation",
			animation);

	Ref<Animation> anim = model->get_animation_tree()->get_animation(animation);

	out->duration = anim->get_length();
	out->position = 0.0;

	current_sequence = out;

	return out;
}

void BipedAnimationBase::abort_sequence(Ref<AnimationSequenceFuture> p_animation_sequence) {
	if (p_animation_sequence == current_sequence && p_animation_sequence.is_valid() && p_animation_sequence->status != BipedAnimationSequenceFuture::FINISHED) {
		p_animation_sequence->status = BipedAnimationSequenceFuture::AnimationSequenceFutureStatus::ABORTED;
		current_sequence.unref();
	}
}
...
void BipedAnimationBase::_update_upper_body_state() {
	BipedCharacterUpperBodyState state;
	...
	switch (weapon_animation_set) {
		case WEAPON_ANIMATION_TYPE_NONE: {
			state = BipedCharacterUpperBodyState::RIFLE_IDLE;
			blend = 0.0f;
		} break;
		case WEAPON_ANIMATION_TYPE_RIFLE: {
			if (is_aiming) {
				...
				blend = 1.0f;
				state = BipedCharacterUpperBodyState::RIFLE_AIM;
			} else {
				blend = 0.0f;
				state = BipedCharacterUpperBodyState::RIFLE_IDLE;
			}
			if (current_sequence.is_valid() && current_sequence->sequence_slot == BipedCharacterAnimationSequenceSlot::SEQUENCE_SLOT_UPPER_BODY) {
				const StringName node_length = String(current_sequence->node_path) + "/current_length";
				const StringName node_pos = String(current_sequence->node_path) + "/current_position";

				const float length = model->get_animation_tree()->get(node_length);
				const float current = model->get_animation_tree()->get(node_pos);
				current_sequence->position = current;
				if (current >= length && get_current_upper_body_animation_state() == UPPER_BODY_STATE_SEQUENCE) {
					// Animation finished, RIP
					print_line("DONE!", length, node_length);
					current_sequence->status = AnimationSequenceFuture::AnimationSequenceFutureStatus::FINISHED;
					current_sequence.unref();
				} else {
					state = UPPER_BODY_STATE_SEQUENCE;
				}
			}
		} break;
