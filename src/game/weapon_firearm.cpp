...
void WeaponFirearmInstance::post_update(int p_weapon_slot, BaseCharacter *p_character, const WeaponButtonState &p_button_state) {
    ...
    if (reloading && next_possible_primary_attack <= time) {
        reloading = false;
    ...
    if (p_button_state.fire && next_possible_primary_attack <= time) {
        if (!has_ammo_in_clip(p_character)) {
            if (p_character->get_remaining_ammo_in_pool(get_ammo_type()) > 0) {
                ...
                if (are_we_player) {
                    p_character->begin_reload();
                    reloading = true;
                    print_line("RELOAD BEGIN!", p_character->get_reload_duration());
                    next_possible_primary_attack = time + p_character->get_reload_duration();
                    return;
                }
            } else {
...
int WeaponFirearmInstance::get_clip_capacity() const { return 10; }
