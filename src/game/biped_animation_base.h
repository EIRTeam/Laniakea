...
enum BipedCharacterUpperBodyState {
	RIFLE_IDLE,
	RIFLE_AIM,
	UPPER_BODY_STATE_SEQUENCE,
	UPPER_BODY_STATE_MAX
};

enum BipedCharacterAnimationSequenceSlot {
	SEQUENCE_SLOT_UPPER_BODY
};

class BipedAnimationSequenceFuture : public AnimationSequenceFuture {
	GDCLASS(BipedAnimationSequenceFuture, AnimationSequenceFuture);

public:
	BipedCharacterAnimationSequenceSlot sequence_slot;
	String node_path;
...
public:
	enum CharacterWeaponAnimations {
		AIM,
		IDLE,
		RELOAD,
		WEAPON_ANIMATION_MAX
	};
...
protected:
	...
	String get_upper_body_animation_state_string(const BipedCharacterUpperBodyState p_state) const;

	Ref<BipedAnimationSequenceFuture> current_sequence;
...
public:
	...
	BipedCharacterUpperBodyState get_current_upper_body_animation_state() const;
	void set_upper_body_animation_state(const BipedCharacterUpperBodyState p_state) const;
	...
	virtual Ref<AnimationSequenceFuture> trigger_sequence(const CharacterAnimationSequence p_sequence) override;
	virtual void abort_sequence(Ref<AnimationSequenceFuture> p_animation_sequence) override;
};
