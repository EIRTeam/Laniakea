...
void LNVehicleEngineSettings::_ensure_torque_curve_baked() {
	...
	for (int i = 1; i < arr.size(); i++) {
		...
		if (!Engine::get_singleton()->is_editor_hint()) {
			...
			if (torque_entry.size() != 2) {
				continue;
			}
			if (torque_entry[0].is_valid_float()) {
				continue;
			}
			if (torque_entry[1].is_valid_float()) {
				continue;
			}
		}
...
float LNVehicleEngineSettings::inverse_sample_torque(float p_torque_ratio, float p_rpm) const {
	if (p_torque_ratio >= 1.0f) {
		return 1.0f;
	}
	if (p_torque_ratio <= 0.0f) {
		return 0.0f;
	}
	...
	for (int i = 0; i < 4; ++i) {
		...
		if (Math::abs(err) < 1e-7f) {
			break;
		}
		if (Math::abs(dr) < 1e-10f) {
			break;
		}
