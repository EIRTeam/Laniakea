...
Ref<Mesh> LODMesh::generate_mesh(const int p_mesh_quality) {
	...
	for (int x = 0; x < vertices_per_side - 1; x++) {
		int top_idx = x;
		int bottom_idx = (x == 0) ? corner_bl : (x == vertices_per_side - 1) ? corner_br
																			 : skirt_vertex_start + x - 1;
		int next_top_idx = x + 1;
		int next_bottom_idx = (x == vertices_per_side - 2) ? corner_br : skirt_vertex_start + x;
	...
	for (int x = 0; x < vertices_per_side - 1; x++) {
		...
		int bottom_idx = (x == 0) ? corner_tl : (x == vertices_per_side - 1) ? corner_tr
																			 : top_row_start + x - 1;
		int next_top_idx = top_idx + 1;
		int next_bottom_idx = (x == vertices_per_side - 2) ? corner_tr : top_row_start + x;
	...
	for (int y = 0; y < vertices_per_side - 1; y++) {
		...
		int bottom_idx = (y == 0) ? corner_bl : (y == vertices_per_side - 1) ? corner_tl
																			 : left_row_start + y - 1;
		int next_top_idx = (y + 1) * vertices_per_side;
		int next_bottom_idx = (y == vertices_per_side - 2) ? corner_tl : left_row_start + y;
	...
	for (int y = 0; y < vertices_per_side - 1; y++) {
		...
		int bottom_idx = (y == 0) ? corner_br : (y == vertices_per_side - 1) ? corner_tr
																			 : right_row_start + y - 1;
		int next_top_idx = (y + 1) * vertices_per_side + (vertices_per_side - 1);
		int next_bottom_idx = (y == vertices_per_side - 2) ? corner_tr : right_row_start + y;
